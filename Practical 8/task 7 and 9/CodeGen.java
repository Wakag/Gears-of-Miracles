...
import java.util.*;
  ...
  class CodeGen {
    ...
    public static void dis_3(){ //task 9
      emit(PVM.dis_3);
    } //CodeGen.dis_3

    public static void sqr(){  //task 7
      emit(PVM.sqr);
    } //CodeGen.sqr

    public static void sqrt(){ //task 7
      emit(PVM.sqrt);
    } //CodeGen.sqrt

    public static void startLoop(boolean to){ //task 9 
      if(to) {emit(PVM.stl)} else emit(PVM.sdl) ;
      emit(adr.address());
    }

    public static void endLoop(boolean to){ //task 9
      if(to) {emit(PVM.etl)} else emit(PVM.edl) ;
      emit(adr.address());
    }
