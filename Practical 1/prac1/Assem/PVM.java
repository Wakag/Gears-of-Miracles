      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.mul:           // integer multiplication	     
		    //ADDED CODE FOR TASK4 - gives information on error
            tos = pop();		  
			sos = pop();
			if (tos != 0 && sos > maxInt / tos) //check overflow
				ps = badVal ;     // update program status
			else
				push(sos * tos);
            break;
          case PVM.div:           // integer division (quotient) 
            tos = pop();
			if (tos == 0)		   //ADDED CODE FOR TASK4 - gives information on error
				ps = divZero ;    //change program status
			else
				push(pop() / tos);
            break;
          case PVM.rem:           // integer division (remainder)
            tos = pop();          //ADDED CODE FOR TASK4 - gives information on error
			if (tos == 0)
				ps = divZero ;    //update program status
			else 
				push(pop() % tos);
            break;
          ...
          case PVM.inpc:          // character input 
			adr = pop();          //CODE CHANGED FOR TASK5
            if (inBounds(adr)) {
          ...
          case PVM.prnc:          // character output
			if (tracing) results.write(padding);   //CODE ADDED FOR TASK5
            results.write((char) pop(), 0); 
            if (tracing) results.writeLine();
            break;
          case PVM.cap:           // toUpperCase
			push((int) Character.toUpperCase(((char) pop()))); 
			break;
          case PVM.low:           // toLowerCase //CODE ADDED FOR TASK5
			tos = pop();
              if(65 <= tos && tos <= 90){
                push(tos + 32);
              }
              ...
              break;
          case PVM.islet:		  // isLetter  //CODE ADDED FOR TASK5
			tos = pop();
              if((65 <= tos && tos <= 90) || (97 <= tos && tos <= 122)){
                push(1);
              }
              else{
                push(0);
              ...
              break;      // isLetter
          case PVM.inc:           // ++ //CODE ADDED FOR TASK5
			tos = pop();
            push(tos + 1);
            break;
          case PVM.dec:           // --	//CODE ADDED FOR TASK5
			tos = pop();
            push(tos - 1);
            break;
